-- PRACTICA 5
module Map_v2 (Map, emptyM, assocM, lookUpM, deleteM, domM) where
    ...
    -- Orden (1)
    -- version con clave unica
    assocM :: Eq k => k -> v -> Map k v -> Map k v
    assocM key val (M xs) = if (elem key (domM (M xs))) 
        then assocM key val (deleteM key (M xs))
        else M ((key, val) : xs)
